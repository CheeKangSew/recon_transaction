...
def load_and_prepare_data(file1, file2):
    ...
    df1['Transaction Date'] = pd.to_datetime(df1['Transaction Date'], format='%m/%d/%Y', errors='coerce')
    df1['Transaction Time'] = pd.to_datetime(df1['Transaction Time'], format='%I:%M:%S %p', errors='coerce').dt.time
    df1.dropna(subset=['Transaction Date', 'Transaction Time'], inplace=True)
    ...
    # Handle multiple possible vehicle column names for df1
    vehicle_column = None
    for col in df1.columns:
        if 'Vehicle' in col and 'No' in col:
            vehicle_column = col
            break
    if vehicle_column is None:
        raise KeyError("Vehicle registration number column not found in file1.")
    
    # Debug print to verify detected column name
    print(f"Detected vehicle column in file1: {vehicle_column}")
    
    # Filter necessary columns for matching
    df1_filtered = df1[['TransactionDateTime', 'Amount (RM)', vehicle_column, 'Receipt No.']]

    # Rename columns for clarity
    df1_filtered.rename(columns={'Amount (RM)': 'Amount1', vehicle_column: 'VehicleNumber1', 'Receipt No.': 'ReceiptNo'}, inplace=True)
...
def match_transactions(df1_filtered, df2_filtered, time_buffer_hours=1):
    ...
    matched_transactions = pd.DataFrame(columns=['TransactionDateTime', 'Amount1', 'VehicleNumber1', 'Amount2', 'VehicleNumber2', 'TransactionNo', 'ReceiptNo'])
...
def match_transactions1(df2_filtered, df1_filtered, time_buffer_hours=1):
    # Create an empty DataFrame to store matched transactions
    matched_transactions1 = pd.DataFrame(columns=['TransactionDateTime', 'Amount2', 'VehicleNumber2', 'Amount1', 'VehicleNumber1', 'ReceiptNo', 'TransactionNo'])
...
def main():
    ...
    if file1 and file2:
        ...
        df2 = df2.merge(matched_transactions1[['TransactionDateTime', 'ReceiptNo']], on='TransactionDateTime', how='left')
        ...
        st.subheader("Petron File with Matched Column, Mismatch Reasons, and TransactionNo")
        st.dataframe(df1)

        # Download button for updated file1
        st.download_button(
            label="Download Petron File with TransactionNo",
        ...
        st.download_button(
            label="Download Soliduz File with Receipt No.",
